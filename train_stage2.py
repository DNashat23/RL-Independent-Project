"""
Stage 2 Curriculum Training — 6 healthy cells, no unhealthy cells, static board.

Loads Stage 1 weights and continues training on a harder version of the task.
The agent must now find and collect 6 healthy cells instead of 3.

Promotion requires:
  - ep_rew_mean >= PROMOTION_THRESHOLD (~collecting most of 6 cells on average)
...
N_ENVS              = 4
MAX_EPISODE_STEPS   = 300        # more steps needed for 6 cells
PROMOTION_THRESHOLD = 5.0        # ~collecting 5 of 6 cells on average
STAGE1_MODEL_PATH   = "ppo_stage1"
MODEL_SAVE_PATH     = "ppo_stage2"
OUTPUT_DIR          = "stage2"   # all PNGs saved here

PPO_KWARGS = dict(
    policy        = "MlpPolicy",
    n_steps       = 2048,
    batch_size    = 64,
    learning_rate = 1e-4,        # lower LR: fine-tuning on top of Stage 1
    ent_coef      = 0.01,
    verbose       = 1,
)
...
# ── Stage 2 environment ───────────────────────────────────────────────────────
class Stage2Env(GridHealthEnv):
    """6 healthy cells, no unhealthy cells. Static board within episode."""
    N_HEALTHY   = 6
    N_UNHEALTHY = 0
...
def train(base_dir):
    print("=" * 60)
    print("  STAGE 2: 6 healthy cells, static board, no unhealthy")
    print(f"  Loading Stage 1 weights from: {STAGE1_MODEL_PATH}.zip")
    print(f"  Timesteps:           {TOTAL_TIMESTEPS:,}")
    print(f"  Max steps/episode:   {MAX_EPISODE_STEPS}")
    print(f"  Learning rate:       {PPO_KWARGS['learning_rate']} (fine-tuning)")
    print(f"  Promotion threshold: ep_rew_mean >= {PROMOTION_THRESHOLD}")
    print("=" * 60)

    stage1_path = os.path.join(base_dir, STAGE1_MODEL_PATH)
    if not os.path.exists(stage1_path + ".zip"):
        print(f"\n✗ Stage 1 model not found at {stage1_path}.zip")
        print("  Run train_stage1.py first and confirm promotion.")
        sys.exit(1)

    vec_env = make_vec_env(
        lambda: TimeLimit(Stage2Env(), max_episode_steps=MAX_EPISODE_STEPS),
        n_envs=N_ENVS
    )

    # Load Stage 1 weights, attach Stage 2 environment
    model = PPO.load(
        stage1_path,
        env=vec_env,
...
def plot_learning_curve(logger, save_path):
    ...
    fig.suptitle("Stage 2 — Learning Curves (6 Healthy Cells, Static Board)",
    ...
    ax1.axhline(6.0, color="gold", linestyle=":", linewidth=1.2,
                label="Perfect score (6.0)")
...
def plot_heatmap(trajectory, save_path, label="Episode"):
    ...
    ax.set_title(f"Stage 2 — Heatmap — {label} ({len(trajectory)} steps)",
                 fontweight="bold")
...
def plot_replay(trajectory, save_path_prefix, label="Episode", cols=10):
    ...
    for png_idx, chunk in enumerate(chunks):
        ...
        fig.suptitle(f"Stage 2 — {label}  (Steps {start_t}–{end_t})",
...
def run_episodes(model, n_seeds=50, max_steps=300):
    all_results = []
...
def evaluate(model, n_episodes=50):
    ...
    for seed in range(n_episodes):
        ...
        collected_counts.append(
            Stage2Env.N_HEALTHY - int(np.sum(env.grid == 1.0))
        )
        env.close()
    ...
    print(f"  Mean cells collected: {np.mean(collected_counts):.2f} "
          f"/ {Stage2Env.N_HEALTHY}")
    print(f"  All-collected rate:   "
          f"{np.mean(np.array(collected_counts)==Stage2Env.N_HEALTHY)*100:.0f}%")
...
if __name__ == "__main__":
    ...
    plot_learning_curve(
        logger,
        save_path=os.path.join(output_dir, "stage2_learning_curve.png")
    )
    ...
    plot_heatmap(best_traj,
                 os.path.join(output_dir, "stage2_heatmap_best.png"),
                 label=f"Best (reward={best_rew:.1f})")
    plot_heatmap(worst_traj,
                 os.path.join(output_dir, "stage2_heatmap_worst.png"),
                 label=f"Worst (reward={worst_rew:.1f})")
    plot_replay(best_traj,
                os.path.join(output_dir, "stage2_replay_best"),
                label=f"Best Episode (reward={best_rew:.1f})")
    plot_replay(worst_traj,
                os.path.join(output_dir, "stage2_replay_worst"),
    ...
    print("=" * 60)
    print(f"  Please review files in: {output_dir}/")
    print()
    while True:
        answer = input("  Advance to Stage 3? (y/n): ").strip().lower()
        if answer in ("y", "n"):
            break
        print("  Please enter y or n.")

    if answer == "y":
        print("\n✓ Promotion confirmed. Run train_stage3.py to continue.")
    else:
        print("\n✗ Promotion declined. Adjust and retrain Stage 2.")